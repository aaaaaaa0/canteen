...
# Универсальная функция для отправки контактной информации
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="⬅️",
                        callback_data=f"menu_section_{product_data.menu_section_id}"
                    ),
                    InlineKeyboardButton(
...
async def to_main_menu_handler(callback: CallbackQuery):
    await callback.message.answer(
        "Вы в главном меню", reply_markup=kb.main_keyboard
    )
    await callback.answer()
...
async def to_section_menu_handler(callback: CallbackQuery):
    await callback.message.answer(
        "Выберите раздел меню:", reply_markup=await kb.menu_sections()
    )
    await callback.answer()
...
async def process_quantity(message: types.Message, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="⬅️ К разделу меню",
                    callback_data=f"show_menu_sections_{menu_section_id}"
                )
            ]
        ]
    )
...
async def process_delete_position(message: Message, state: FSMContext):
    ...
    product_id_to_remove = cart_items[position_number - 1][0]  # (id, name, price, quantity)
...
async def start_registration(callback: CallbackQuery):
    text = (
        "[ ВАЖНО! ]\n\n"
        "⁘ Перед оформлением заказа, пожалуйста, проверьте контактные данные.\n\n"
        "⁘ Также убедитесь, что все товары и их количество в корзине корректны.\n\n"
        "⁘ После подтверждения заказа вам будет выслан чек для оплаты."
    )
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Все данные верны\nПодтвердить заказ", callback_data="confirm_order")]
    ])
...
async def confirm_order(callback: CallbackQuery, state: FSMContext):
    ...
    # Подтверждение заказа и генерация order_id
    order_id = await rq.confirm_order(tg_id)

    # Отправка уведомления админ-боту
